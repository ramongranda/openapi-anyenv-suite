...
if (args.length === 0) {
  console.error(
    "Usage: pnpm run report -- <path/to/openapi.yaml> [--port 8080] [--regenerate]"
  );
  process.exit(2);
}
const file = args[0];
...
try {
  ...
  const gradeScript = path.join(__dirname, 'grade.mjs');
    ...
    console.log(`${existsSync(reportJsonPath) ? 'dist/grade-report.json present but regeneration requested' : 'dist/grade-report.json not found'}, running check to (re)generate artifacts`);
  ...
  await run('pnpm', checkArgs);
  } else {
    console.log('Using existing dist/grade-report.json produced by `check` — skipping re-run.');
  }
  ...
  try {
    ...
    console.warn('Could not copy assets to dist/assets:', String(copyErr?.message || copyErr));
  }
  ...
  if (generateOnly || !existsSync(docsPathEarly)) {
      try {
        ...
        if (existsSync(path.join(process.cwd(), 'dist', 'bundled.json'))) {
          try { embedded = JSON.parse(await (await import('node:fs/promises')).readFile(path.join(process.cwd(),'dist','bundled.json'), 'utf8')); } catch(e) { embedded = null; }
        }
        ...
        writeFileSync(docsPathEarly, redocHtml, 'utf8'); console.log('Created fallback dist/docs.html (ReDoc)');
      } catch (e) { console.warn('Could not create fallback docs.html:', e?.message || e); }
    }
  if (generateOnly || !existsSync(swaggerPathEarly)) {
      try {
        ...
        writeFileSync(swaggerPathEarly, swaggerHtml, 'utf8'); console.log('Created fallback dist/swagger.html (Swagger UI)');
      } catch (e) { console.warn('Could not create fallback swagger.html:', e?.message || e); }
    }
  } catch (e) {
    console.warn('Could not ensure early fallback docs/swagger pages:', e?.message || e);
  }

  if (!report) {
    console.log('No JSON report found; creating minimal HTML placeholder.');
    writeFileSync(reportHtmlPath, '<html><body><h1>OpenAPI Grade Report</h1><p>No JSON report available.</p></body></html>');
  } else {
    ...
    if (Array.isArray(spectral.problems)) {
      ...
      const errs = spectral.problems.filter(p => Number(p.severity) >= 1).length;
      const warns = spectral.problems.filter(p => Number(p.severity) < 1).length;
    ...
    if (Array.isArray(redocly.problems)) {
      const errs = redocly.problems.filter(p => String(p.severity || '').toLowerCase().startsWith('err')).length;
      const warns = redocly.problems.filter(p => String(p.severity || '').toLowerCase().startsWith('warn')).length;
    ...
    if (sp.length > 0) {
      spectralSection = `<div class="overflow-x-auto"><table class="min-w-full text-sm table-auto border-collapse divide-y divide-slate-700"><thead class="bg-slate-900"><tr class="text-left"><th class="px-3 py-2"></th><th class="px-3 py-2 text-slate-300">Severity</th><th class="px-3 py-2 text-slate-300">Code</th><th class="px-3 py-2 text-slate-300">Message</th><th class="px-3 py-2 text-slate-300">Location</th></tr></thead><tbody class="bg-transparent">` +
        sp.map(i => {
          ...
          if (sl.startsWith('err')) { sevClass = 'sev-error'; badgeClass += ' bg-rose-600 text-rose-50'; }
          else if (sl.startsWith('warn')) { sevClass = 'sev-warning'; badgeClass += ' bg-amber-500 text-amber-900'; }
          ...
          const loc = JSON.stringify(i.path || i.location || i.range || i.source || '');
          // Use encoded data-* attributes to avoid breaking HTML when values contain quotes
          return `<tr class="issue-row ${sevClass} odd:bg-slate-800/60" data-severity="${escAttr(sev)}" data-code="${escAttr(code)}" data-path="${escAttr(loc)}" data-message="${escAttr(msg)}"><td class="px-3 py-2"><input type="checkbox" class="sel" aria-label="Select issue"/></td><td class="${tdBaseClass}"><span class="${badgeClass}">${escHtml(sev)}</span></td><td class="px-3 py-2 font-mono">${escHtml(code)}</td><td class="px-3 py-2">${escHtml(msg)}</td><td class="px-3 py-2"><code>${escHtml(loc)}</code></td></tr>`;
    ...
    if (Array.isArray(redocly.problems) && redocly.problems.length > 0) {
      redoclySection = `<div class="overflow-x-auto"><table class="min-w-full text-sm table-auto border-collapse divide-y divide-slate-700"><thead class="bg-slate-900"><tr class="text-left"><th class="px-3 py-2"></th><th class="px-3 py-2 text-slate-300">Severity</th><th class="px-3 py-2 text-slate-300">Code</th><th class="px-3 py-2 text-slate-300">Message</th><th class="px-3 py-2 text-slate-300">Location</th></tr></thead><tbody class="bg-transparent">` +
        redocly.problems.map(p => {
          // Normalize severity
          let sev = '';
          try { const s = p.severity; if (typeof s === 'string') sev = s; else if (Number(s) === 0) sev = 'error'; else if (Number(s) === 1) sev = 'warning'; else sev = String(s ?? ''); } catch(_) { sev = String(p.severity ?? ''); }
          ...
          } catch (e) { loc = JSON.stringify(p.location || p.path || ''); }
          ...
          try {
            const isPointer = typeof loc === 'string' && (loc.startsWith('#') || loc.startsWith('/') || loc.includes('~1') || loc.includes('~0'));
            if (isPointer) {
              let pstr = String(loc || '');
              // remove leading '#'
              if (pstr.startsWith('#')) pstr = pstr.slice(1);
              try { pstr = decodeURIComponent(pstr); } catch (e) {}
              ...
              if (pstr.length > 1 && pstr.endsWith('/')) pstr = pstr.slice(0, -1);
              loc = pstr;
            }
          } catch (e) {}
          const sl = String(sev || '').toLowerCase();
          ...
          return `<tr class="issue-row ${sevClass} odd:bg-slate-800/60" data-severity="${escAttr(sev)}" data-code="${escAttr(rule)}" data-path="${escAttr(loc)}" data-message="${escAttr(msg)}"><td class="px-3 py-2"><input type="checkbox" class="sel" aria-label="Select issue"/></td><td class="px-3 py-2"><span class="${badgeClass}">${escHtml(sev)}</span></td><td class="px-3 py-2 font-mono">${escHtml(rule)}</td><td class="px-3 py-2">${escHtml(msg)}</td><td class="px-3 py-2"><code>${escHtml(loc)}</code></td></tr>`;
        }).join('') + `</tbody></table></div>`;
    } else if (redocly && redocly.raw && (redocly.errors || redocly.warnings)) {
      const safeRaw = String(redocly.raw).replaceAll('<', '&lt;').replaceAll('>', '&gt;');
    ...
    const tplPath = path.join(__dirname, '..', 'templates', 'grade-report.html');
    if (existsSync(tplPath)) {
      try {
        const tpl = await import('node:fs/promises');
        let template = await tpl.readFile(tplPath, 'utf8');
        // basic replacements
        const gradeBg = (letter === 'A') ? 'bg-emerald-400' : (letter === 'B' ? 'bg-sky-500' : (letter === 'C' ? 'bg-amber-500' : 'bg-rose-600'));
  ...
  template = template.replaceAll('{{logoClass}}', '');
        template = template.replaceAll('{{spectralErrors}}', String(spectral.errors ?? 0));
        template = template.replaceAll('{{spectralWarnings}}', String(spectral.warnings ?? 0));
        template = template.replaceAll('{{redoclyErrors}}', String(redocly.errors ?? 0));
        template = template.replaceAll('{{redoclyWarnings}}', String(redocly.warnings ?? 0));
  // expose whether redocly was available so the client can disable the toggle
    template = template.replaceAll('{{redoclyAvailable}}', String(redocly.available === true));
    ...
    const redoclyBtnClass = redoclyEnabledDefault ? 'bg-sky-600' : 'bg-slate-700';
    const redoclyKnobClass = redoclyEnabledDefault ? 'translate-x-6' : 'translate-x-1';
    const redoclyDisabledAttr = redocly.available === false ? 'disabled' : '';
    const redoclyDisabledClasses = redocly.available === false ? 'opacity-50 pointer-events-none' : '';
    template = template.replaceAll('{{redoclyAriaPressed}}', redoclyAriaPressed);
    template = template.replaceAll('{{redoclyBtnClass}}', redoclyBtnClass);
    template = template.replaceAll('{{redoclyKnobClass}}', redoclyKnobClass);
    template = template.replaceAll('{{redoclyDisabledAttr}}', redoclyDisabledAttr);
    template = template.replaceAll('{{redoclyDisabledClasses}}', redoclyDisabledClasses);
        template = template.replaceAll('{{gradeBg}}', gradeBg);
        template = template.replaceAll('{{gradeText}}', gradeText);
        template = template.replaceAll('{{year}}', String(new Date().getFullYear()));
  ...
  template = template.replaceAll('{{REDOCLY_SECTION}}', redoclySection);
        ...
        if (existsSync(aiTplPath)) {
          ...
          try {
            ...
            const fmtPct = (n) => (typeof n === 'number' ? String(Math.round(n * 100) + '%') : String(n ?? '0%'));
            const replacements = {
            ...
            for (const k of Object.keys(replacements)) aiRaw = aiRaw.replaceAll(k, replacements[k]);
          } catch (e) {
        ...
        try {
          ...
          try {
            const docsExists = existsSync(path.join(process.cwd(), 'dist', 'docs.html'));
            const bundleExists = existsSync(path.join(process.cwd(), 'dist', 'bundled.json'));
        ...
        try {
          ...
          const presence = heur.presence || {};
          const fmtPct = (n) => (typeof n === 'number' ? String(Math.round(n * 100) + '%') : String(n ?? '0%'));
          template = template.replaceAll('{{operations}}', String(totals.operations ?? 0));
          template = template.replaceAll('{{summaryPct}}', fmtPct(ratios.withSummary));
          template = template.replaceAll('{{descPct}}', fmtPct(ratios.withDesc));
          template = template.replaceAll('{{with4xxPct}}', fmtPct(ratios.with4xx));
          template = template.replaceAll('{{opIdUniquePct}}', fmtPct(ratios.opIdUniqueRatio));
          template = template.replaceAll('{{hasTitle}}', presence.hasTitle ? 'Yes' : 'No');
          template = template.replaceAll('{{hasVersion}}', presence.hasVersion ? 'Yes' : 'No');
          template = template.replaceAll('{{hasServers}}', presence.hasServers ? 'Yes' : 'No');
          template = template.replaceAll('{{hasSecSchemes}}', presence.hasSecSchemes ? 'Yes' : 'No');
        ...
        try {
          const rebuildHtml = generateOnly ? '' : `<button id="rebuildBtn" class="px-2 py-1 rounded bg-rose-700 hover:bg-rose-600 inline-flex items-center gap-2" title="Rebuild report">
  ...
  if (generateOnly || !existsSync(docsPath)) {
      try {
        ...
        const bundleScript = embedded ? `\n<script>window.__BUNDLED_SPEC__ = ${JSON.stringify(embedded)};</script>` : '';
        const redocHtml = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>API Docs</title><script src="https://cdn.jsdelivr.net/npm/redoc@next/bundles/redoc.standalone.js"></script></head><body style="margin:0;padding:0"><div id="redoc-container"></div>${bundleScript}<script> (function(){ try{ if(window.__BUNDLED_SPEC__){ Redoc.init(window.__BUNDLED_SPEC__, {}, document.getElementById('redoc-container')); } else { const r = document.createElement('redoc'); r.setAttribute('spec-url','bundled.json'); document.body.appendChild(r); } }catch(e){ console.error('ReDoc init failed', e); } })();</script></body></html>`;
        writeFileSync(docsPath, redocHtml, 'utf8'); console.log('Created fallback dist/docs.html (ReDoc)');
      } catch (e) { console.warn('Could not create fallback docs.html:', e?.message || e); }
    }
  ...
  if (generateOnly || !existsSync(swaggerPath)) {
      try {
        ...
        const bundleScript = embedded ? `\n<script>window.__BUNDLED_SPEC__ = ${JSON.stringify(embedded)};</script>` : '';
        const swaggerHtml = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Swagger UI</title><link rel="stylesheet" href="https://unpkg.com/swagger-ui-dist@4/swagger-ui.css" /></head><body style="margin:0;padding:0"><div id="swagger"></div><script src="https://unpkg.com/swagger-ui-dist@4/swagger-ui-bundle.js"></script>${bundleScript}<script> (function(){ try{ if(window.__BUNDLED_SPEC__){ SwaggerUIBundle({ spec: window.__BUNDLED_SPEC__, dom_id: '#swagger' }); } else { SwaggerUIBundle({ url: 'bundled.json', dom_id: '#swagger' }); } }catch(e){ console.error('Swagger init failed', e); } })();</script></body></html>`;
        writeFileSync(swaggerPath, swaggerHtml, 'utf8'); console.log('Created fallback dist/swagger.html (Swagger UI)');
      } catch (e) { console.warn('Could not create fallback swagger.html:', e?.message || e); }
    }
  } catch (e) {
    console.warn('Could not ensure fallback docs/swagger pages:', e?.message || e);
  }
  ...
  // Start Vite to serve the `dist/` directory. Use pnpm exec so local vite is used.
  await run('pnpm', ['exec', 'vite', '--root', 'dist', '--port', String(port)]);
} catch (e) {
