...
export default async function validate() {
  try {
    ...
    try {
      await run(resolveBin('redocly'), ['bundle', file, '--output', outPath, '--ext', 'json']);
    } catch (e) {
      console.error('redocly bundle failed, falling back to local bundler:', e?.message ?? e);
      try { await run(process.execPath, ['scripts/bundle.mjs', file, '--out', outPath]); }
      catch (e2) { console.error('local bundler failed:', e2?.message ?? e2); throw e; }
    }

    if (!existsSync(outPath)) {
      console.warn('dist/bundled.json missing after bundling attempts; trying to normalize alternative artifacts');
      try {
        ...
        if (candidate) {
          ...
          if (lower.endsWith('.json')) copyFileSync(src, outPath);
            ...
            const content = readFileSync(src, 'utf8');
            try { const parsed = JSON.parse(content); writeFileSync(outPath, JSON.stringify(parsed, null, 2), 'utf8'); }
            catch (_) { try { const yaml = (await import('yaml')).default; const parsed = yaml.parse(content); writeFileSync(outPath, JSON.stringify(parsed, null, 2), 'utf8'); } catch (_) { copyFileSync(src, outPath); } }
          }
        }
      } catch (normErr) { console.error('Normalization failed:', normErr?.message ?? normErr); }
    }

    const spectralRuleset = path.resolve(path.dirname(new URL(import.meta.url).pathname), '..', '.spectral.yaml');
    console.log(`Spectral lint (bundle only): ${outPath}`);
    await run(resolveBin('spectral'), ['lint', outPath, '--ruleset', spectralRuleset, '--fail-severity', 'error']);
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith('validate-core.mjs')) {
  await validate();
}
