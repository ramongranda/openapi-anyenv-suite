...
export function computeHeuristics(spec) {
  ...
  const hasSecSchemes = !!Object.keys(spec.components?.securitySchemes ?? {})
    .length;
  ...
  if (withSummary >= config.bonuses.rules.summaryRatio.threshold)
    bonus += config.bonuses.rules.summaryRatio.points;
  if (withDesc >= config.bonuses.rules.descriptionRatio.threshold)
    bonus += config.bonuses.rules.descriptionRatio.points;
  if (with4xx >= config.bonuses.rules["4xxRatio"].threshold)
    bonus += config.bonuses.rules["4xxRatio"].points;
