...
export function calculateScore(spectral, redocly, heuristics) {
  let score = 100;
  score -= Math.min(
    config.penalties.spectral.maxError,
    spectral.errors * config.penalties.spectral.error
  );
  score -= Math.min(
    config.penalties.spectral.maxWarning,
    spectral.warnings * config.penalties.spectral.warning
  );
  if (redocly) {
    score -= Math.min(
      config.penalties.redocly.maxError,
      redocly.errors * config.penalties.redocly.error
    );
    score -= Math.min(
      config.penalties.redocly.maxWarning,
      redocly.warnings * config.penalties.redocly.warning
    );
  }
