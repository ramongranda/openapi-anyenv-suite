...
// Provide __dirname equivalent for ESM tests
...
test("grade-npx.mjs produces report using NPX shim", () => {
  ...
  const res = spawnSync(
    process.execPath,
    [join(process.cwd(), "scripts", "grade-npx.mjs"), "spec.yaml"],
...
// Escribir grade.config.json en el cwd temporal antes de ejecutar el script
// para que el proceso hijo lo encuentre correctamente
// (debe ir después de definir cwd)
...
test("grade-npx.mjs produces report using NPX shim", () => {
  ...
  // Escribir la configuración en el cwd temporal
